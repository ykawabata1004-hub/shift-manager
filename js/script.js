...
class Store {
    ...
    updateApprovalStatus(periodId, userId, status) {
        ...
        // Save to Firebase
        this.db.ref('/approvals').set(this.state.approvals);
    }
    ...
    updateMemo(date, text) {
        ...
        // Save to Firebase
        this.db.ref('/memos').child(date).set(text);
    }
    ...
    createPeriod(startDate, endDate) {
        ...
        // Save to Firebase
        this.db.ref('/periods').set(this.state.periods);
        this.db.ref('/entries').set(this.state.entries);
        this.db.ref('/approvals').set(this.state.approvals);
        if (!this.state.currentPeriodId) {
            this.db.ref('/currentPeriodId').set(newPeriod.id);
        }
        return newPeriod;
    }
    ...
    updateUser(userId, newName) {
        ...
        if (user) {
            user.name = newName;
            // Save to Firebase
            this.db.ref('/users').set(this.state.users);
        }
    }
    ...
    setActivePeriod(periodId) {
        if (this.state.periods.find(p => p.id === periodId)) {
            ...
            // Save to Firebase
            this.db.ref('/currentPeriodId').set(periodId);
        }
    }

    async resetData() {
        // Clear Firebase data and reinitialize
        await this.db.ref('/').set({
            users: INITIAL_USERS,
            entries: SEED_DATA.entries,
            periods: SEED_DATA.periods,
